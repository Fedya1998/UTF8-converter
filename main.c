 ...
 * @param blocks - ptr to number of 8 bit blocks. Calc in func.
 * @param x - input number
 * @return encoded number
 */
uint8_t *to_utf8(uint8_t *blocks, const uint32_t *x);
...
uint8_t *to_utf8(uint8_t *blocks, const uint32_t *x) {
	if(*x <= 0x7F) *blocks = 1;
	else if(*x <= 0x7FF) *blocks = 2;
	else if(*x <= 0xFFFF) *blocks = 3;
	else if(*x <= 0x1FFFFF) *blocks = 4;
	else {
	...
	switch(*blocks) {
		...
		case 4:
			...
			res[3] = a | 0x80;
			break;
		default: printf("Error: in func to_utf8: wrong blocks\n");
			 exit(1);
			 break;
	}
	return res;
...
uint32_t from_utf8(const uint8_t *x) {
	...
	if((x[0] >> 3) == 0x1E) {
		...
		printf("Zhopa in from_utf8: wrong format\n");
		exit(1);
	}
